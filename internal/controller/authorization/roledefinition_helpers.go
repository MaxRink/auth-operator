...
func (r *RoleDefinitionReconciler) buildFinalRules(
	...
	// /metrics is a non-resource URL, so only ClusterRoles can carry it. Keep
	// metrics access opt-in and respect get restrictions.
	if roleDefinition.Spec.MetricsAccessAllowed &&
		roleDefinition.Spec.TargetRole == authorizationv1alpha1.DefinitionClusterRole &&
