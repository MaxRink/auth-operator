...
func (r *BindDefinitionReconciler) reconcileResources(
	...
	logger.V(2).Info("reconcileResources: Ensuring ServiceAccounts",
		"bindDefinition", bindDefinition.Name,
		"subjectCount", len(bindDefinition.Spec.Subjects))
	previousGeneratedSAs := append([]rbacv1.Subject(nil), bindDefinition.Status.GeneratedServiceAccounts...)
...
func bindDefinitionDesiredServiceAccounts(subjects []rbacv1.Subject) map[string]struct{} {
	desired := make(map[string]struct{}, len(subjects))
	for _, subject := range subjects {
		...
		desired[subject.Namespace+"/"+subject.Name] = struct{}{}
	}
	return desired
}
...
func (r *BindDefinitionReconciler) pruneStaleServiceAccounts(
	...
	trustedGeneratedSAs []rbacv1.Subject,
) error {
	logger := log.FromContext(ctx)

	var pruneErrors []error
	seen := make(map[string]struct{}, len(trustedGeneratedSAs))
	for _, subject := range trustedGeneratedSAs {
		if subject.Kind != authorizationv1alpha1.BindSubjectServiceAccount {
			continue
		}
		if subject.Namespace == "" || subject.Name == "" {
		...
		sa := &corev1.ServiceAccount{}
		err := r.client.Get(ctx, types.NamespacedName{Namespace: subject.Namespace, Name: subject.Name}, sa)
		if apierrors.IsNotFound(err) {
			continue
		}
		if err != nil {
			pruneErrors = append(pruneErrors, fmt.Errorf("get stale ServiceAccount candidate %s: %w", key, err))
			continue
		}
		if !hasOwnerRef(sa, bindDef) {
			logger.V(1).Info("skipping previously generated ServiceAccount without current ownerRef",
				"namespace", sa.Namespace, "name", sa.Name, "bindDefinition", bindDef.Name)
			continue
		}
		logger.Info("pruning stale ServiceAccount", "namespace", sa.Namespace, "name", sa.Name, "bindDefinition", bindDef.Name)
		if _, err := r.deleteServiceAccount(ctx, bindDef, sa.Name, sa.Namespace); err != nil {
			pruneErrors = append(pruneErrors, fmt.Errorf("delete stale ServiceAccount %s: %w", key, err))
			continue
		}
	}
...
// subjects or previously recorded as generated by this BindDefinition.
func (r *BindDefinitionReconciler) deleteSubjectServiceAccounts(
	ctx context.Context,
	bindDef *authorizationv1alpha1.BindDefinition,
) error {
	logger := log.FromContext(ctx)
	subjects := bindDefinitionServiceAccountsForDeletion(bindDef)
	logger.V(2).Info("processing subjects for deletion",
		"bindDefinitionName", bindDef.Name, "subjectCount", len(subjects))
...
func bindDefinitionServiceAccountsForDeletion(bindDef *authorizationv1alpha1.BindDefinition) []rbacv1.Subject {
	...
	for _, subject := range bindDef.Status.GeneratedServiceAccounts {
		add(subject)
	}
	return subjects
}
